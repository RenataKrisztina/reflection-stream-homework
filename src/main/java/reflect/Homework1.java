...
public class Homework1 {
    ...
    public void streamPipeline4() {
        Arrays.stream(java.lang.String.class.getDeclaredMethods()).map(t -> t.getReturnType()).distinct().sorted(Comparator.comparing(t -> t.getName())).forEach(System.out::println);
    }
    ...
    public void streamPipeline5() {
        Arrays.stream(java.lang.String.class.getDeclaredMethods()).filter(c -> Arrays.stream(c.getParameterTypes()).anyMatch(p -> p == boolean.class)).sorted(Comparator.comparing(c -> c.getName())).forEach(System.out::println);
    }
    ...
    public void streamPipeline6() {
        Arrays.stream(java.lang.String.class.getDeclaredMethods()).filter(c -> Arrays.stream(c.getParameterTypes()).allMatch(p -> p == int.class) && c.getParameterCount() != 0).sorted(Comparator.comparing(c -> c.getName())).forEach(System.out::println);
    }
    ...
    public String streamPipeline7() {
        ...
        return Arrays.stream(java.lang.String.class.getDeclaredMethods()).map(s -> s.getName()).max(Comparator.comparingInt(n -> n.length())).get();
    }
